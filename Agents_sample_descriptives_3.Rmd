...
The data were collected using a self-report questionnaire. There were `r pct_miss(mva)` values missing altogether (not including open ended questions), none of the variables showed more than 5% missing. 
...
Based on the non-parametric test of heteroscedasticity, the assumption of missing completely at random (MCAR) was rejected for the numeric data (p = `r MCAR$pnormality`), but the overall amount of missing data in the dataset is very small. 
...
^[geofi package was used: Markus Kainu, Joona Lehtomäki, Juuso Parkkinen, Jani Miettinen, Leo Lahti Retrieval and analysis of open geospatial data from Finland with the geofi R package. R package version 0.9.2900006. URL: http://ropengov.github.io/geofi]

```{r include = F}
data %<>% 
  mutate(occu=factor(occu, labels = c("Studying", "Working", "Army/civil service", "Other"))) %>%
  mutate(schl=if_else(is.na(schl) & q14 > 1, 0, schl)) %>% 
